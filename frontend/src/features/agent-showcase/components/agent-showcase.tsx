import { AgentShowcaseBoundary } from "./agent-showcase-boundary";

const sceneDescription = "AgentWhatsApp AI assistant answering a Moroccan customer and confirming an order";

type AgentShowcaseProps = Readonly<{
  id?: string;
}>;

export function AgentShowcase({ id }: AgentShowcaseProps) {
  return (
    <aside
      aria-label={sceneDescription}
      className="relative isolate hidden w-full min-w-0 md:max-w-xl md:justify-self-center md:block lg:-ml-12 lg:w-[calc(100%_+_5rem)] lg:max-w-none lg:justify-self-stretch xl:-ml-16 xl:w-[calc(100%_+_6rem)]"
      id={id}
    >
