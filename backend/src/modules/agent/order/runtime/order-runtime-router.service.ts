...
import { runConfirmedOrderReceiptPreview } from "../confirmed-order/confirmed-order-preview.service";
...
function isStableAction(message: string): boolean {
  return /^(first_entry:|cart_offer:|cart_quantity:|cart_item_option:|cart_item_previous:|cart_review:|cart_review_item:|cart_review_item_edit:|order_checkout:|order_checkout_field:)/.test(message);
}
...
function asTurnResult(input: RuntimeReply & { stage: OrderRuntimeStage; warnings?: readonly string[]; failureCode?: string; confirmedSnapshotId?: string; receiptReady?: boolean }): OrderRuntimeTurnResult {
  return {
    handled: true,
    reply: input.text,
...
export async function processGuardedOrderRuntimeTurn(input: OrderRuntimeTurnInput): Promise<OrderRuntimeTurnResult> {
  ...
  if (runtime.runtimeStage === "CONFIRMED") {
    return asTurnResult({ text: "الطلب ديالك تأكد من قبل. شكراً على الثقة ديالك.", stage: "CONFIRMED" });
  }
  ...
  if (runtime.runtimeStage === "PLANNING") {
    const planning = runCartPlanningPreview({ previewEnabled: true, rawActionId: isStableAction(message) ? message : undefined, planningText: !isStableAction(message) ? message : undefined, sellerId: input.sellerId, productContext, offerLookup: offers, cart: runtime.cart, previewPlanningState: runtime.planningState, now });
    if (!planning.handled) return isStableAction(message) ? asTurnResult({ ...staleActionReply(), stage: runtime.runtimeStage }) : { handled: false, warnings: [] };
    ...
    runtime.cart = planning.cartAfter;
    runtime.planningState = planning.previewPlanningState;
    runtime.lastHandledAction = isStableAction(message) ? message : undefined;
  ...
  if (runtime.runtimeStage === "COLLECTING_ITEM") {
    const item = runItemCollectionPreview({ previewEnabled: true, rawActionId: isStableAction(message) ? message : undefined, itemCollectionText: !isStableAction(message) ? message : undefined, sellerId: input.sellerId, productContext, requiredFields: fields, cart: runtime.cart, previewState: runtime.itemCollectionState });
    if (!item.handled) return isStableAction(message) ? asTurnResult({ ...staleActionReply(), stage: runtime.runtimeStage }) : { handled: false, warnings: [] };
  ...
  if (runtime.runtimeStage === "CART_REVIEW" || runtime.runtimeStage === "EDITING_CART_ITEM") {
    const review = runCartReviewPreview({ previewEnabled: true, rawActionId: isStableAction(message) ? message : undefined, cartReviewText: !isStableAction(message) ? message : undefined, sellerId: input.sellerId, productContext, requiredFields: fields, offerLookup: offers, cart: runtime.cart, previewState: runtime.cartReviewState, cartItemEditPreviewState: runtime.itemEditState, now });
    if (!review.handled) return isStableAction(message) ? asTurnResult({ ...staleActionReply(), stage: runtime.runtimeStage }) : { handled: false, warnings: [] };
    runtime.cart = review.cartAfter;
    runtime.cartReviewState = review.previewState;
    ...
    if (review.nextStep === "DELIVERY_COLLECTION_READY") {
      const delivery = runDeliveryConfirmationPreview({ previewEnabled: true, sellerId: input.sellerId, conversationScopeId: input.conversationKey, productContext, requiredFields: fields, offerLookup: offers, cart: runtime.cart, now });
      runtime.cart = delivery.cartAfter;
      runtime.deliveryConfirmationState = delivery.previewState;
      runtime.runtimeStage = "COLLECTING_DELIVERY";
      await persist(input, runtime, fields);
  ...
  if (runtime.runtimeStage === "COLLECTING_DELIVERY" || runtime.runtimeStage === "FINAL_ORDER_REVIEW") {
    const delivery = runDeliveryConfirmationPreview({ previewEnabled: true, rawActionId: isStableAction(message) ? message : undefined, deliveryConfirmationText: !isStableAction(message) ? message : undefined, previewState: runtime.deliveryConfirmationState, sellerId: input.sellerId, conversationScopeId: input.conversationKey, productContext, requiredFields: fields, offerLookup: offers, cart: runtime.cart, now, cartReviewPreviewState: runtime.cartReviewState, cartItemEditPreviewState: runtime.itemEditState });
    if (!delivery.handled) return isStableAction(message) ? asTurnResult({ ...staleActionReply(), stage: runtime.runtimeStage }) : { handled: false, warnings: [] };
    runtime.cart = delivery.cartAfter;
    runtime.deliveryConfirmationState = delivery.previewState;
    if (delivery.nextStep === "RETURN_TO_CART_REVIEW") runtime.runtimeStage = "CART_REVIEW";
      ...
      const receipt = await runConfirmedOrderReceiptPreview({ previewEnabled: true, cart: runtime.cart, previewState: delivery.previewState, confirmedPreview: delivery.confirmedPreview, sellerId: input.sellerId, conversationScopeId: input.conversationKey, productContext, requiredFields: fields, offerLookup: offers, receiptContext: { storeName: sellerConfig.businessName, paymentMethodLabel: sellerConfig.receipt.paymentMethodLabel, deliveryText: sellerConfig.delivery.text }, now, snapshotId: `runtime-${input.conversationKey.replace(/[^a-zA-Z0-9_-]/g, "_")}`, confirmedAt: now.toISOString() });
      if (!receipt.success || !receipt.snapshot || !receipt.receiptDocument) return asTurnResult({ ...recoveryReply(), stage: runtime.runtimeStage, warnings: receipt.warnings, failureCode: receipt.failureCode });
      runtime.runtimeStage = "CONFIRMED";
      runtime.confirmed = { snapshotId: receipt.snapshot.id, status: "CONFIRMED", receipt: receipt.receiptDocument, confirmedAt: receipt.snapshot.confirmedAt };
      await persist(input, runtime, fields);
      return asTurnResult({ text: "تم تأكيد الطلب ديالك. غادي نتواصلو معاك قريباً.", stage: "CONFIRMED", warnings: receipt.warnings, confirmedSnapshotId: receipt.snapshot.id, receiptReady: true });
    }
    await persist(input, runtime, fields);
