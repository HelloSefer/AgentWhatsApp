...
class FakeMetaTransport implements MetaEmbeddedSignupTransport {
  ...
  async inspectToken(): Promise<MetaTokenInspectionResult> {
    throw new Error("Phase 11E1 does not inspect tokens.");
  }
...
function fixture(status: WhatsAppConnectionStatus = "VERIFYING") {
  ...
  return credentialService.storeAccessToken(tenant, candidate.connectionId, { accessToken }).then(() => ({
    ...
    candidate,
    accessToken,
    service: new WhatsAppConnectionFinalizationService(repository, credentialService, transport),
  }));
}
...
async function main(): Promise<void> {
  ...
  add("Successful finalization registers phone and subscribes WABA", result.finalized && result.connection.status === "VERIFYING" && result.connection.phoneRegistrationCompleted && result.connection.wabaSubscriptionCompleted && happy.transport.registerCalls === 1 && happy.transport.subscriptionCalls === 1);
  ...
  for (const status of ["ACTIVE", "DISCONNECTED", "REVOKED", "PENDING"] as const) {
  ...
  await activePreserved.service.finalize(activePreserved.tenant, activePreserved.candidate.connectionId);
  add("Previous ACTIVE connection remains unchanged", activePreserved.repository.connections.find((entry) => entry.connectionId === previousActive.connectionId)?.status === "ACTIVE");
