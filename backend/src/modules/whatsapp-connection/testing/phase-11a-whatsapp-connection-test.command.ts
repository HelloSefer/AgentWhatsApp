...
async function main(): Promise<void> {
  ...
  try {
    ...
    add("WhatsApp connection table contains the Phase 11A and nullable onboarding persistence shape", columns.rows.map((row) => row.column_name).join("|") === "connection_id|seller_id|provider|status|meta_business_id|waba_id|phone_number_id|display_phone_number|verified_name|connected_at|last_verified_at|disconnected_at|created_at|updated_at|encrypted_access_token|token_key_version|token_fingerprint|token_expires_at|encrypted_registration_pin|registration_pin_key_version|registration_pin_fingerprint|phone_registration_completed_at|waba_subscription_completed_at|finalization_last_error_code|finalization_last_error_at|replaced_connection_id|connection_method|meta_app_id|public_webhook_id|encrypted_meta_app_secret|meta_app_secret_key_version|encrypted_system_user_access_token|system_user_access_token_key_version|encrypted_webhook_verify_token|webhook_verify_token_key_version|bound_product_id");
  } finally {
    await cleanup();
