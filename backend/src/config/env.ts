...
export const env = {
  ...
  authEmailSmtpHost: (process.env.AUTH_EMAIL_SMTP_HOST || "").trim(),
  authEmailSmtpPort: Number(process.env.AUTH_EMAIL_SMTP_PORT) || 0,
  authEmailSmtpSecure: (process.env.AUTH_EMAIL_SMTP_SECURE || "").trim().toLowerCase() === "true",
  authEmailSmtpUser: (process.env.AUTH_EMAIL_SMTP_USER || "").trim(),
  authEmailSmtpPassword: process.env.AUTH_EMAIL_SMTP_PASSWORD || "",
  authEmailFromName: (process.env.AUTH_EMAIL_FROM_NAME || "").trim(),
  authEmailFromAddress: (process.env.AUTH_EMAIL_FROM_ADDRESS || "").trim(),
